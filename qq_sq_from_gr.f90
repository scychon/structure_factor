...
!        write(6,*) 'volume', volume, trajin % box(1,1), trajin % box(2,2), trajin % box(3,3)
 ...
 100    FORMAT(A, I8, A)  
    ...
    !$OMP PARALLEL &
    !$OMP   DEFAULT (FIRSTPRIVATE) &
    !$OMP   SHARED (sq,sq2,sqPart,sqlorch,sqlorch2,chargeAtType,grsum)
    !$OMP DO
    do k=1, MAX_Q_FORM
      kval = k*kunit
      do j=1, nattypepairs
        ...
        sqval = invvol * npartInAtType(attype1) * npartInAtType(attype2) * chargeAtType(attype1) * chargeAtType(attype2)
        if (k==1) then
...
!    enddo
    ...
    do k=1, MAX_Q_FORM
      ...
      if ( k .eq. 1) write(6,'(F12.3,4ES15.7)') k*kunit, sq(k), sqlorch(k)
    enddo
    close(17)
