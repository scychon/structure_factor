...
! ----------------------------------------------------------------
  ...
  write(6,'(A,A)') 'reading topology file ',trim(strTopFile)
  write(6,*) ''
...
!            write(6,*) comtraj(ixtc,idxmol+j,:)
        ...
        if ( mod(ixtc,nskipgr) == 0) then
            ...
            !$OMP PARALLEL &
            !$OMP   DEFAULT (FIRSTPRIVATE) &
            !$OMP   SHARED (grframesum,grsum)
            !$OMP DO
            do i=1,minridx
                grsum(i,nattypepairs+1) = grsum(i,nattypepairs+1) + grframesum(i, nattypepairs+1) 
                do j=1,nattypepairs
                    grsum(i,j) = grsum(i,j) + grframesum(i,j) 
                enddo
            enddo
            !$OMP END DO
            !$OMP END PARALLEL
 ...
 100    FORMAT(A, I8, A)  
    ...
    !$OMP PARALLEL &
    !$OMP   DEFAULT (FIRSTPRIVATE) &
    !$OMP   SHARED (sq,sq2,sqPart,sqPartDirect,sqlorch,sqlorch2,chargeAtType,grsum)
    !$OMP DO
    do k=1, MAX_Q_FORM
      kval = k*kunit
      do j=1, nattypepairs
        ...
        sqval = invvol * chargeAtType(attype1) * chargeAtType(attype2)
        sqval2 = sqval * npartInAtType(attype1) * npartInAtType(attype2)
        if (k==1) then
            write(6,*) attype1, attype2, chargeAtType(attype1),chargeAtType(attype2)
        endif
        do i=1, minridx-1
          ival = i + 0.5d0
          rval = dr_bins * ival
          ...
          sqPartDirect(k,j) = sqPartDirect(k,j) + sqval * grout(i,j) * dsin( kval * rval) / (kval * rval)
          sq(k) = sq(k) + dr_bins * sqval * grout(i,j) * dsin( kval * rval) / (kval * rval)
          sqPart(k,j) = sqPart(k,j) + dr_bins * sqval2 * sqbinpre * (grsum(i,j) - 1.d0)
          sq2(k) = sq2(k) + dr_bins * sqval2 * sqbinpre * (grsum(i,j) - 1.d0)
          sqlorch(k) = sqlorch(k) + dr_bins * sqval2 * sqbinpre2 * (grsum(i,j) - 1.d0)
        enddo
      enddo
    enddo
    !$OMP END DO
    !$OMP END PARALLEL
    write(strfmt,'("( F12.6, ",I0,"ES15.7 )")') (nattypepairs+3)
    open(17,file=strSqFile)
    write(17,*) "# k       S(k)       S(k)_Lorch         "
    write(6,*) "# k       S(k)       S(k)_Lorch          "
    do k=1, MAX_Q_FORM
      write(17,strfmt) k*kunit, sq(k), sqlorch(k), sqPart(k,:)
      if ( k .eq. 1) write(6,'(F12.3,4ES15.7)') k*kunit, sq(k), sq2(k), sqlorch(k)
    enddo
    close(17)
